...
### 🔴 Bug critique : la branche de production Vercel n'était pas `main`
- **Erreur commise puis corrigée en session** : juste après avoir branché le domaine, j'ai affirmé au propriétaire que `main` était la branche de production, en me basant sur `list_deployments` qui montrait un déploiement récent avec `githubCommitRef: "main"` en état READY. **C'était une inférence fausse** — Vercel crée un déploiement (preview) pour CHAQUE branche poussée, avec son propre alias `takalamu-git-<branche>-....vercel.app` ; la présence d'un déploiement récent depuis `main` ne prouve rien sur la config "Production Branch" du projet. Il fallait regarder le champ `target` (`"production"` vs `null`/`"preview"`) ou l'écran "Production Deployment" du dashboard, pas juste la fraîcheur/branche du dernier item de la liste.
- **Le propriétaire a détecté l'anomalie lui-même** (vitrine publique encore visible sur `tatakalamu.fr` alors qu'elle devait être supprimée depuis la session 22) et a insisté pour vérifier — réflexe à adopter : ne jamais affirmer qu'un merge sur `main` "est en prod" sans avoir vérifié `target: "production"` sur le déploiement le plus récent de ce commit précis via `list_deployments`.
- **Root cause confirmée** : Production Deployment réel = commit du 23 juin (`407d576`), branche `claude/new-project-setup-1jcgwf` — jamais mise à jour depuis (cf. session 5, `lessons.md` plus bas, qui l'avait déjà documenté et qui n'a manifestement pas été relu/appliqué depuis). 93 commits de retard, tout le pivot de service absent de la prod pendant des semaines.
- **Réglage "Production Branch" introuvable dans l'app mobile Vercel** (`Settings → Git`, scrollé intégralement : Connected Repository → PR/Commit Comments → Git Commits → Git LFS → Deploy Hooks, aucune carte "Production Branch" entre les deux). Probablement une limitation de l'app mobile vs. le site complet — non vérifié faute de temps, le propriétaire n'avait accès qu'au mobile dans l'instant.
- **Fix appliqué (proposé par le propriétaire, la bonne réponse)** : plutôt que de chercher le réglage, faire pointer directement la branche de prod sur `main` : `git push --force-with-lease origin origin/main:refs/heads/claude/new-project-setup-1jcgwf`. Un `--is-ancestor` a d'abord confirmé que ce n'était pas un simple fast-forward (le vieux commit n'est pas un ancêtre de `main` — il datait d'avant une réécriture massive) donc force push nécessaire ; sans risque ici car `main` est la version de référence "tout est à jour". Vérifié après coup via `list_deployments` : nouveau déploiement avec `target: "production"` sur le commit de `main`, état passé à READY.
- **Limite non résolue** : ce fix est un **rattrapage ponctuel**, pas une correction du réglage racine. Sans changer "Production Branch" vers `main` dans les réglages Vercel (site web complet, pas l'app), **chaque nouveau merge sur `main` ne partira PAS automatiquement en prod** — il faudra soit resynchroniser `claude/new-project-setup-1jcgwf` à la main à chaque fois, soit trouver et corriger le réglage. Averti dans `CLAUDE.md` §11 pour qu'aucune session future ne suppose l'inverse.
