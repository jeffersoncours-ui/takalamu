          ...
          editing === b.id ? (
            <EditRow
              ...
              onSave={async (vals) => {
                ...
                try {
                  ...
                  if (vals.coverFile) coverUrl = await uploadCover(vals.coverFile);
                  const res = await updateBook(b.id, { title: vals.title, subtitle: vals.subtitle, coverUrl });
                  if (res.error) setError(res.error);
                  else {
                    setEditing(null);
                    router.refresh();
            ...
            >
              ...
              <button
                ...
                style={{ background: "#F7F4EE", color: "#4A463F", border: "1px solid #E9E3D8" }}
              >
                Modifier
              </button>
              <button
                type="button"
                onClick={() => {
                  if (confirm(`Supprimer le livre « ${b.subtitle || b.title} » ?`)) run(() => deleteBook(b.id));
                }}
                disabled={pending || busy}
                className="shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold disabled:opacity-50"
                style={{ background: "#FDECEC", color: RED, border: "1px solid #F3B0B2" }}
              >
                Suppr.
              </button>
            </div>
...
function EditRow({
  ...
  book: Book;
  busy: boolean;
  onSave: (vals: { title: string; subtitle: string; coverFile?: File }) => void;
  onCancel: () => void;
}) {
  ...
  return (
    <div className="rounded-[16px] p-3 space-y-2.5" style={{ background: "#FBF9F5", border: "1px solid #E9E3D8" }}>
      <input value={title} onChange={(e) => setTitle(e.target.value)} dir="rtl" className="font-arabic" style={inputStyle} />
      <input value={subtitle} onChange={(e) => setSubtitle(e.target.value)} placeholder="Libellé français" style={inputStyle} />
      <label className="flex items-center gap-2 text-sm cursor-pointer" style={{ color: "#8B857A" }}>
      ...
      <div className="flex gap-2">
        <button
          type="button"
          onClick={() => onSave({ title, subtitle, coverFile: coverRef.current?.files?.[0] })}
          disabled={busy || !title.trim()}
          className="flex-1 rounded-[11px] py-2.5 text-sm font-semibold text-white disabled:opacity-50"
          style={{ background: GREEN }}
        >
        ...
        <button
          ...
          onClick={onCancel}
          disabled={busy}
          className="rounded-[11px] px-4 py-2.5 text-sm font-semibold"
