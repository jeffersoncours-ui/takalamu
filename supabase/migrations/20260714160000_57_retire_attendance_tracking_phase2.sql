-- Phase 2 (destructive) du retrait du suivi de présence — À APPLIQUER
-- SEULEMENT une fois le déploiement prod de la phase 1 confirmé (base
-- partagée preview/prod : le client encore déployé enverrait sinon
-- p_attendance à une RPC qui ne l'accepte plus, ou lirait une colonne
-- absente). Retire p_attendance des deux RPC (DROP+CREATE, plus aucun
-- client ne l'envoie), la colonne lesson_records.attendance, l'enum
-- attendance_status, et students.unjustified_absences_count.

DROP FUNCTION public.submit_session_record(
  uuid, timestamptz, attendance_status, text, text, text, jsonb, jsonb, jsonb, jsonb, text, uuid
);
...
BEGIN
  ...
  INSERT INTO public.lesson_records
    (student_id, teacher_id, session_date, public_recap, support_files, custom_title, course_group_id)
  VALUES
    (p_student_id, v_teacher_id, p_session_date,
...
DROP FUNCTION public.update_session_record(
  uuid, timestamptz, attendance_status, text, text, text, jsonb, jsonb, jsonb, jsonb, text
);

CREATE FUNCTION public.update_session_record(
  p_record_id uuid,
  p_session_date timestamp with time zone,
  p_public_recap text DEFAULT NULL::text,
  p_private_note text DEFAULT NULL::text,
...
ALTER TABLE public.lesson_records DROP COLUMN attendance;
DROP TYPE attendance_status;
ALTER TABLE public.students DROP COLUMN unjustified_absences_count;
