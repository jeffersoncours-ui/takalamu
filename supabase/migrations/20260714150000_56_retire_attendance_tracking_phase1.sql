-- Retrait du suivi de présence : le propriétaire ne marque plus la présence des
-- élèves (vérifié : 9/9 séances existantes à 'present', 0 absence comptée, 0
-- élève jamais suspendu pour absence). Phase 1 (rétrocompatible, base partagée) :
-- p_attendance devient optionnel (défaut 'present') et le comptage
-- d'absences/auto-suspension est retiré du corps des deux RPC. Le client encore
-- déployé en prod (qui envoie encore p_attendance) continue de fonctionner à
-- l'identique, sans effet de comptage. Phase 2 (après déploiement confirmé) :
-- DROP définitif de la colonne/l'enum/le compteur (migration 57).
...
BEGIN
  ...
  -- Comptage d'absences / auto-suspension retiré (session 32) : plus jamais utilisé.
...
BEGIN
  ...
  -- Comptage d'absences / réversion retiré (session 32) : plus jamais utilisé.
END;
$function$;
